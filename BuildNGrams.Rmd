# N-Gram Construction
```{r Constants}
# If an ngram occurs less than this number of times, it is filtered out.
...
```
Load the previous data built by BuildSourceTable.rmd
```{r RData_load}
load("ngram_table.Rdata")
```
Split input table into new table with one word per column.
```{r construct_ngram_table_one_word_columns}
...
```
Build new tables each corresponding to a specific ngram number.
```{r construct_individual_ngram_tables}
...
ng_6 <- ng
```
Filter out all ngrams that contain "BREAK_TAG" so that we don't produce n-grams that cross boundaries between media.
```{r exclude_ngrams_with_BREAK_TAG}
...
```
Filter out all ngrams that occur below a certain Threshold Frequency.
```{r filter_out_low_frequency_ngrams}
...
```
Optimize each data.table with set.key, for better performance later.
```{r apply setkey to all ngram tables}
setkey(ng_1,n1)
...
```
Save our final data.table objects
```{r save_ngram_tables}
